 ...
 *
 * 1. This matches the `quota` value described in `vmadm(8)`. A default value
...
function buildFilesystemZonecfg(vmobj, payload, options)
{
    ...
    for (filesystem in add) {
        ...
        // Triton will provide UUIDs for us, but for standalone SmartOS
        // we'll support a magic string to indicate that this volume doesn't
        // exist yet.
        if (filesystem.source === 'create-volume') {
            // Set the source to a uuid so that we'll trigger on the next
            // if statement and the volume will get created.
            filesystem.source = libuuid.create();
        }
...
function installZone(payload, log, callback)
{
    ...
    async.series([
        function (cb) {
             ...
             * Apply ZFS settings
             *
             * These are run as separate invocations of zfs(8) to make
            ...
            // the zoneroot. This does not work with HVMs.
            if (receiving
                || reprovisioning
                || !payload.hasOwnProperty('add_filesystems')
                || vmobj.hvm) {
            ...
            payload.add_filesystems.forEach(function (filesystem) {
                ...
                // Insanely, HTTPS URLs are implemented. Hindsight is 20/20,
                // but they are a world of hurt. We're sorry, we didn't know.
                // Limit this damage to docker.
                /* JSSTYLED */
                } else if (vmobj.docker && filesystem.source.match(/^https?:\/\//)) {
...
function deleteZone(uuid, log, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            /*
             * We want to prevent VMs which have volumes that are shared
             ...
             */
            if (vmobj.hvm || ! vmobj.filesystems
            ...
            if (data_volumes.length === 0) {
                log.debug({uuid: vmobj.uuid}, 'VM has no local data volumes '
                    + 'no need to check other VMs for dependent filesystems');
                cb();
                return;
            }
            ...
            VM.lookup({}, {fields: [
                ...
                if (err) {
                    log.error({err: err, uuid: vmobj.uuid}, 'failed to lookup '
                        + 'potential dependent filesystems');
                    cb(err);
                    return;
                }
                ...
                log.error({uuid: vmobj.uuid, volume_users: volume_users},
                    'found VMs using dependent filesystems, cannot delete');
    ...
    VM.load(uuid, {fields: load_fields, log: log}, function (err, vmobj) {
        ...
        vasync.pipeline({
            funcs: [
                ...
                function rebuildCheckpoints(_, next) {
                    var opts = {
                        log: log,
                        vmobj: vmobj
                    };
                    VM.sync_checkpoints(uuid, opts, next);
                }
            ]
        }, callback);
    });
...
exports.sync_checkpoints = function(uuid, options, callback)
{
    var found;
    var lockpath;
    var log;
    ...
    if (options.hasOwnProperty('log')) {
        ...
        log = VM.log.child({action: 'rebuild-checkpoints', vm: uuid});
    }
    ...
    if (process.env.EXPERIMENTAL_VMJS_TRACING) {
        tracers_obj = traceUntilCallback('rebuild-checkpoints', log, callback);
        callback = tracers_obj.callback;
        log = tracers_obj.log;
    }
    ...
    assert.uuid(uuid, 'uuid');
    assert.object(options, 'options');
    assert.object(blacklist, 'blacklist');
    assert.func(callback, 'callback');
    assert(log, 'log');
    ...
    vasync.waterfall([
        function lock(cb) {
        ...
        }, function checkVmTypeAndState(cb) {
            if (vmobj.brand === 'kvm') {
            ...
            if (vmobj.brand === 'bhyve') {
                var err = new Error('bhyve vms skip');
                err.skip = true;
                cb(err);
                return;
            }
            if (vmobj.state !== 'running' &&
                options.unmountOnly === undefined &&
                options.blacklist === undefined) {

                var err = new Error('stopped vms skip');
                err.skip = true;
                cb(err);
                return;
            }
            ...
            snapdir = vmobj.zonepath + '/.zfs/snapshot';

            if (options.unmountOnly === true) {
                cb();
                return;
            }
            ...
            zfs(args, log, function onList(zfs_err, fds) {
                ...
                fds.stdout.split('\n').forEach(function (snapshot) {
                    ...
                    if (parts.length !== 2 || parts[1] === '')
                        return;
                    if (parts[1].indexOf('vmsnap-') === 0)
                        snaps[parts[1]] = true;
                    if (parts[1].indexOf('weekly-') === 0)
                        snaps[parts[1]] = true;
                    if (parts[1].indexOf('daily-') === 0)
                        snaps[parts[1]] = true;
                    if (parts[1].indexOf('hourly-') === 0)
                        snaps[parts[1]] = true;
                });

                log.debug('snapshots = %j', Object.keys(snaps));

                cb();
            });
        }, function listCheckpoints(cb) {
            ...
            fs.readdir(mountpoint, function (e, files) {
                if (e && e.code === 'ENOENT') {
                    cb();
                    return;
                }
                ...
                vasync.forEachParallel({
                    inputs: files,
                    func: function (file, cb2) {
                        fs.lstat(mountpoint + '/' + file, function (e2, stat) {
                            if (e2) {
                                checkpoints[file] = {status: 'bad'};
        ...
        }, function findMountStatus(cb) {
            fs.readFile('/etc/mnttab', {encoding: 'utf-8'}, function (e, data) {
                if (e) {
                    cb(e);
                    return;
                }
                data.split('\n').forEach(function (line) {
                    ...
                    var special = parts[0], path = parts[1], fstype = parts[2];
                    var mntopts = parts[3];
                    if (!special || !path || !fstype)
                        return;
                    if (fstype === 'lofs' &&
                        path.indexOf(mountpoint) === 0 &&
                        special.indexOf(snapdir) === 0) {

                        var slug = path.slice(mountpoint.length + 1);
                        var sdirs = special.split('/');
                        if (sdirs[sdirs.length - 1] === 'root' &&
                            sdirs[sdirs.length - 2] === slug) {

                            if (!checkpoints[slug])
                                checkpoints[slug] = {status: 'ok'};
                            checkpoints[slug].mounted = true;
                            delete (mounts[slug]);
                        } else {
                            checkpoints[slug] = {status: 'bad', mounted: true};
                            delete (mounts[slug]);
                        }
                    } else if (path.indexOf(mountpoint) === 0) {
                        var slug = path.slice(mountpoint.length + 1);
                        checkpoints[slug] = {status: 'bad', mounted: true};
                        delete (mounts[slug]);
                    }
                });
                Object.keys(mounts).forEach(function (slug) {
                    checkpoints[slug] = {status: 'bad'};
                });
                log.debug('checkpoints = %j', checkpoints);
                cb();
            });
        }, function unmountOld(cb) {
            var toUnmount = Object.keys(checkpoints).filter(function (k) {
                if (checkpoints[k].status === 'bad' &&
                    checkpoints[k].mounted)
                    return true;
                if (!snaps[k] && checkpoints[k].mounted)
                    return true;
                if (blacklist[k] && checkpoints[k].mounted)
                    return true;
                return false;
            });
            /*
             * Try an ordinary "umount" without -f first on all the mounted
             * snapshots. If any fail, retry them in the next step with -f,
             * and if that fails, give up.
             */
            log.debug('unmounting %j', toUnmount);
            vasync.forEachParallel({
                inputs: toUnmount,
                func: function (slug, cb2) {
                    var cmd = '/usr/sbin/umount';
                    var argv = [mountpoint + '/' + slug];
                    traceExecFile(cmd, argv, log, 'umount-snapshot',
                        function (e, stdout, stderr) {

                        if (e) {
                            log.trace({err: e}, 'There was an error while '
                                + 'unmounting the snapshot: ' + e.message);
                            // We treat an error here as an error only if it
                        ...
                        log.trace('umounted ' + slug);
                        // Mark this one off, we won't try to force-unmount it.
                        checkpoints[slug].mounted = false;
                        cb2();
                    });
                }
            }, function (perr) {
            ...
            log.debug('force-unmounting %j', toUnmount);
            vasync.forEachParallel({
                inputs: toUnmount,
                func: function (slug, cb2) {
                    var cmd = '/usr/sbin/umount';
                    var argv = ['-f', mountpoint + '/' + slug];
                    traceExecFile(cmd, argv, log, 'umount-snapshot',
                        function (e, stdout, stderr) {
        ...
        }, function deleteOld(cb) {
            var toDelete = Object.keys(checkpoints).filter(function (k) {
                if (checkpoints[k].status === 'bad')
                    return true;
                if (!snaps[k])
                    return true;
            ...
            log.debug('deleting %j', toDelete);
            vasync.forEachParallel({
                inputs: toDelete,
                func: function (slug, cb2) {
                    fs.rmdir(mountpoint + '/' + slug, function (e) {
                        if (e && e.code === 'ENOTDIR') {
                            fs.unlink(mountpoint + '/' + slug, cb2);
                            return;
                        } else if (e) {
                            log.debug({err: e}, 'failed deleting %s', slug);
                        } else {
                            log.debug('deleted %s', slug);
                        }
                        cb2();
                    });
                }
        ...
        }, function ensureMountpoint(cb) {
            log.debug('ensuring mountpoint exists');
            fs.mkdir(mountpoint, function (e) {
                if (e && e.code !== 'EEXIST') {
                    cb(e);
                    return;
                }
        ...
        }, function mountNew(cb) {
            var toMount = Object.keys(snaps).filter(function (k) {
                if (checkpoints[k] &&
                    checkpoints[k].status === 'ok' &&
                    checkpoints[k].mounted)
                    return false;
                if (blacklist[k])
                    return false;
            ...
            log.debug('mounting %j', toMount);
            vasync.forEachParallel({
                inputs: toMount,
                func: function (slug, cb2) {
                    var snappath = snapdir + '/' + slug + '/root';
                    var mountpath = mountpoint + '/' + slug;

                    fs.mkdir(mountpath, function (e) {
                        if (e && e.code !== 'EEXIST') {
                            cb2(e);
                            return;
                        }
                        ...
                        var argv = [ '-F', 'lofs', '-o',
                          'ro,nosetuid,nodevices', snappath, mountpath ];
                        traceExecFile(cmd, argv, log, 'mount-snapshot',
                            function (e2, stdout, stderr) {

                            if (e2) {
                                log.error({err: e2}, 'unable to mount ' +
                                    'snapshot: ' + slug + ': ' + e2.message);
                            }
                            cb2();
                        });
                    });
...
exports.delete_snapshot = function (uuid, snapname, options, callback)
{
    ...
    vasync.waterfall([
        function (cb) {
            ...
            /* Update the zone checkpoints dir if it has one */
            var blacklist = {};
            blacklist['vmsnap-' + snapname] = true;
            var opts = {
                log: log,
                locked: true,
                blacklist: blacklist,
            ...
            VM.sync_checkpoints(uuid, opts, cb);
        }, function (cb) {
            var cancelFn;
            vasync.parallel({funcs: [
                function (cb2) {
